...
export class RoomDurableObject {
  ...
  async alarm() {
    ...
    if (!room || room.phase !== "started" || !room.game || room.game.phase !== "night") {
      return;
    }
  ...
  async setNightAlarmIfNeeded(room) {
    if (!room.game || room.game.phase !== "night") {
      return;
    }

    if (room.game.nightDeadlineAt) {
  ...
  buildPrivateGame(room, playerId) {
    ...
    return getNightActionView(room.game, playerIndex);
  }
  ...
  async handleStartGame(room, playerId, isHost) {
    ...
    await this.saveRoom(room);
    await this.setNightAlarmIfNeeded(room);
    await this.broadcastSnapshot();
  }
